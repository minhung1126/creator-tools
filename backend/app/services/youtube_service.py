...
def fetch_video_details(credentials: Credentials, video_ids: List[str]) -> List[Dict[str, Any]]:
    """Fetch detailed info for a list of video IDs (chunked in batches of 50)."""
    if not video_ids:
        return []
...
def update_single_video_metadata(
    ...
    """
    Update a single video's title and description while preserving
    existing snippet properties (categoryId, tags, language settings).
    """
    service = get_youtube_service(credentials)
...
def publish_and_remove_playlist_item(
    ...
    """
    Set a video's privacy status to 'public' and remove it from the playlist.
    The video itself is NOT deleted from the YouTube channel.
    """
    service = get_youtube_service(credentials)
    ...
    if playlist_item_id:
        try:
            ...
            logger.warning("Failed to delete playlist item %s: %s", playlist_item_id, e)
