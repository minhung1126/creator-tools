...
const INSTAGRAM_R2_DOC = 'https://github.com/minhung1126/creator-tools/blob/main/docs/INSTAGRAM_R2_SETUP.md';
  ...
  return <div className="section-gap" style={{ maxWidth: 980 }}>
    ...
    <section className="glass-panel card-padding" style={{ display: 'grid', gap: 16 }}>
      ...
      {connected ? <div style={{ display: 'grid', gap: 10 }}>
        ...
        <div className="info-banner"><CheckCircle2 size={16} /><span>Token 狀態：{account.status === 'reauthorization_required' ? '需要重新授權' : account.status === 'refresh_failed' ? '最近更新失敗' : '正常／會自動更新'}；最近更新：{formatDate(account.last_refreshed_at)}</span></div>
