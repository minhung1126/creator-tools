...
export default function InstagramSettingsPage({ refreshKey = 0 }) {
  ...
  return <div className="section-gap" style={{ maxWidth: 980 }}>
    ...
    <section className="glass-panel card-padding" style={{ display: 'grid', gap: 16 }}><div><h2>Instagram Reels 工作流程預設值</h2><p className="section-desc">以下是 Instagram Reels 發布流程使用的來源資源；雖然 Drive 與 Sheet 由 Google 提供，但不會套用全域共用資源。</p></div><label className="form-group"><span className="form-label">Reels 來源 Google Drive 資料夾 ID／網址</span><input className="form-input" value={form.drive_folder_id || ''} onChange={(e) => setForm({ ...form, drive_folder_id: e.target.value })} /></label><label className="form-group"><span className="form-label">Instagram 內文來源 Google Sheet ID／網址</span><input className="form-input" value={form.spreadsheet_id || ''} onChange={(e) => setForm({ ...form, spreadsheet_id: e.target.value })} /></label><p className="section-desc">Instagram API 版本由後端 release 固定，不由 UI 修改。</p></section>
