...
export default function SettingsPage({ authUser, sysSettings, refreshSettings }) {
  ...
  return <div className="section-gap" style={{ maxWidth: 1000 }}>
    <div><h1 style={{ fontSize: '1.8rem', marginBottom: 6 }}>全域與 Google 設定</h1><p className="section-desc">管理控制台登入、共用 Google 資源與系統資訊。YouTube 播放清單請至 YouTube 設定。</p></div>
    ...
    <div className="glass-panel card-padding" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}><div><h3 style={{ fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: 10 }}><FileSpreadsheet size={20} color="var(--accent)" /> 共用 Google Sheet</h3><p className="section-desc">這是未指定其他來源時的共用預設值，供 Sheet 內容複製與 YouTube 工作流作為 fallback 使用。</p></div><div className="form-group"><label className="form-label"><FileSpreadsheet size={14} /> 預設 Google Sheet 網址或 Spreadsheet ID</label><SourceLinkInput value={formData.default_spreadsheet_id} onChange={(e) => handleChange('default_spreadsheet_id', e.target.value)} sourceType="spreadsheet" /></div></div>
