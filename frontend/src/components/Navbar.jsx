...
export default function Navbar({ activeTab, setActiveTab, authUser, onLogout }) {
  ...
  return <aside className="sidebar"><div><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={{ background: 'linear-gradient(135deg,#6366f1,#ec4899)', padding: 8, borderRadius: 10 }}><Video size={24} /></div><div><h2 style={{ fontSize: '1.15rem' }}>Creator Tools</h2><p style={{ fontSize: '.75rem', color: 'var(--text-muted)' }}>創作者自動化控制台</p></div></div></div>
    <nav style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1, overflowY: 'auto' }}>{item({ id: 'dashboard', label: '儀表板總覽', icon: LayoutDashboard })}{item({ id: 'api_health', label: 'API健康度', icon: Activity })}{group('YouTube', Youtube, youtubeOpen, setYoutubeOpen, youtubeItems)}{group('Sheet', FileSpreadsheet, sheetOpen, setSheetOpen, sheetItems)}{item({ id: 'settings', label: '全域與 Google 設定', icon: Settings })}</nav>
    <div className="sidebar-footer"><div className="glass-panel" style={{ padding: 14, fontSize: '.85rem' }}><strong style={{ color: '#fff' }}>帳號資訊</strong><span className="badge badge-connected" style={{ marginTop: 8 }}><CheckCircle2 size={12} />控制台已登入</span><p style={{ marginTop: 8, wordBreak: 'break-all' }}>{authUser?.email}</p><span className={`badge ${authUser?.youtube_authenticated ? 'badge-connected' : 'badge-disconnected'}`} style={{ marginTop: 8 }}>{authUser?.youtube_authenticated ? 'YouTube 已授權' : 'YouTube 未連結'}</span><button onClick={onLogout} style={{ marginTop: 8, color: '#f87171', background: 'none', border: 0 }}>登出控制台</button></div></div>
  </aside>;
}
